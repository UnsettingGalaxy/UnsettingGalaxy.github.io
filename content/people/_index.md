---
title: 成员
date: 2026-04-02
type: docs
summary: 团队成员
toc: false
---

团队现有教师与博士生如下。

## 吴健雄 {#wu-jianxiong}

- 职称：教授
- 研究方向：软件复用、程序分析、智能软件工程
- 相关发表：[Reusable Software Platform for Large-Scale Systems](/publications/#reusable-software-platform), [Cross-Project Component Mining with LLM Assistance](/publications/#cross-project-component-mining)

## 李明 {#li-ming}

- 职称：博士生
- 研究方向：代码检索、组件发现
- 相关发表：[Reusable Software Platform for Large-Scale Systems](/publications/#reusable-software-platform)

## 张华 {#zhang-hua}

- 职称：博士生
- 研究方向：大模型辅助软件工程
- 相关发表：[Cross-Project Component Mining with LLM Assistance](/publications/#cross-project-component-mining)
